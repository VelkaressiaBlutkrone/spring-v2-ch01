...
- **트랜잭션 설정**: Service 클래스 레벨에 `@Transactional(readOnly = true)`를 기본적으로 선언하여 성능을 최적화하고, 데이터 변경이 필요한 메서드에만 별도로 `@Transactional`을 선언해야 합니다.
- **성능 최적화 (N+1 문제)**: Lazy 로딩된 연관 객체를 조회할 때는 `select` 쿼리가 여러 번 실행되는 것을 방지하기 위해, Repository에 `fetch join`을 사용하는 전용 메서드를 만들어 한 번에 조인해서 가져와야 합니다.
